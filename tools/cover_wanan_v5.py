...
def main():
    ...
    converted_vocals_path = WORKSPACE / "songs" / "converted_vocals" / f"{song_name}_{MODEL_NAME}_hybrid.wav"
    
    # 1. Run Applio Inference with HYBRID F0 and HIGH Protect (for Falsetto)
    print(f"=== Starting Inference for {song_name} (Hybrid + High Protect) ===", flush=True)
    infer_cmd = [
        str(PYTHON_EXE), str(CORE_PY), "infer",
        "--f0_method", "rmvpe",  # Ultimate median F0 to prevent pitch breaking on falsetto
        "--pth_path", str(MODEL_PATH),
        "--index_path", str(INDEX_PATH),
        "--input_path", str(vocals_path),
        "--output_path", str(converted_vocals_path),
        "--export_format", "WAV",
        "--index_rate", "0.2",   # Keep low to allow high notes
        "--protect", "0.5",      # CRITICAL: 0.5 is maximum protection. This stops RVC from destroying breathy falsettos!
        "--pitch", "0"
    ]
    ...
    final_output = WORKSPACE / "outputs" / f"{song_name}_V5_假音修复最终版.wav"
